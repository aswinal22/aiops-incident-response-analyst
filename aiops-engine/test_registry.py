...
def test_service_registry_lifecycle() -> None:
    ...
    try:
        ...
        # Automated Teardown: Clean up created test project from database
        if project_id:
            delete_project(project_id)
            print(f"  [Teardown] Deleted test project {project_id} from Supabase.")
...
def test_dynamic_log_ingestion_routing() -> None:
    ...
    try:
        ...
        if proj_id:
            ...
            print(f"  [Teardown] Deleted test project {proj_id} from Supabase.")
...
def test_multi_repo_mcp_tools() -> None:
    ...
    assert "target-app" in deps or "Service:" in deps or "No microservices registered" in deps
